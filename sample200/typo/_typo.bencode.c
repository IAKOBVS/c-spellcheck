...
const char *sp)
{
	...
	i_inbt(&iter, sp, __carry_length(be, sp));
...
int *klen)
{
	...
	if (be_item) {
		*klen = len;
		i_inbt(be_item, keyin + len, __carry_length(be, keyin + len));
	}
...
b_t *be_item)
{
	...
	if (be_item) {
		i_inbt(be_item, sp, __carry_length(be, sp));
	}
