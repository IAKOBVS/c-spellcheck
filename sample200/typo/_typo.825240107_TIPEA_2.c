...
main()
{
	...
	double cicilanPokok = htgCicilonPakok(pinjaman, tenor);
	double sisaPinjaman = pinjaman;

	/* Loop untuk menampilkan cicilan setiap bulan */
	for (int bulan = 1; bulan <= tenor; bulan++) {
		double cicilanBunga = htgCicilunBanga(sisaPinjaman, sukuBungaTahunan);
		double totalCicilan = htgTotaliicClan(cicilanPokok, cicilanBunga);
