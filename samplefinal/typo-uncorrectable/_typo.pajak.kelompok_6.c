...
htgBiayaJabatan(long gajiBruto)
{
	...
	return Zmn(biayaJabatan, 6000000LL); /*  Pastikan 6000000LL adalah long literal */
}
...
{
	return Zmx(gajiBersih - ptkp, 0LL); /*  Pastikan PKP tidak negatif */
}
...
{
	return Zmx(gajiBruto - pajak, 0LL); /*  Pastikan gaji bersih tidak negatif */
}
...
mnPajak()
{
	char pilihan;
	do {
		long gajiBruto = ZZZZZjaBruto(); /*  Ipt gaji bruto */
		if (gajiBruto == 0)
			continue; /*  Jika ipt gaji bruto tidak valid, ulangi */

		long biayaJabatan = ZZZZZayaJibatan(gajiBruto); /*  Htg biaya jabatan */
		long iuranPns = ZZZZZPns(); /*  Ipt iuran pensiun */
		long gajiBersih = gajiBruto - biayaJabatan - iuranPns; /*  Htg gaji bersih */

		long ptkp = ZZZZZKh(); /*  Htg PTKP berdasarkan status "Belum Menikah" dan tanggungan */
		long pkp = ZZZZZP(gajiBersih, ptkp); /*  Htg PKP */
		long pajak = ZZZZZjak(pkp); /*  Htg pajak berdasarkan trf */
